/*
 * mgui
 * GitHub:https://github.com/yangfeng002/mgui.git
 * Some code copy from Bootstrap v3.0.0
 * */

/* ========================================================================
 * MGUI: jquery.extensions.js
 * ========================================================================
...
(function($, window, undefined) {
    ...
    /* Check jquery */
    if(typeof($) === 'undefined') throw new Error('MGUI requires jQuery');

    // MGUI shared object
    if(!$.mgui) $.mgui = function(obj) {
    ...
    $.fn.callEvent = function(name, event, model) {
        var $this = $(this);
        var dotIndex = name.indexOf('.mgui.');
        ...
        if(model && model.options) {
            ...
            if($.isFunction(func)) {
                e.result = $.mgui.callEvent(func, e, model);
            }
        }
        $this.trigger(e);
        return e;
...
+ function($) {
    ...
    $.fn.button = function(option) {
        return this.each(function() {
            var $this = $(this)
            var data = $this.data('mgui.button')
            var options = typeof option == 'object' && option

            if(!data) $this.data('mgui.button', (data = new Button(this, options)))
    ...
    $(document).on('click.mgui.button.data-api', '[data-toggle^=button]', function(e) {
        var $btn = $(e.target)
...
(function($, undefined) {
    ...
    var NAME = 'mgui.pager'; // model name
    ...
    var Pager = function(element, options) {
        ...
        that.lang  = $.isPlainObject(lang) ? ($.extend(true, {}, LANG[lang.lang || $.mgui.clientLang()], lang)) : LANG[lang];
...
+ function($) {
    ...
    $.fn.tab = function(option) {
        return this.each(function() {
            ...
            if(!data) $this.data(mguiname, (data = new Tab(this)))
            if(typeof option == 'string') data[option]()
        })
    }
    ...
    $(document).on('click.mgui.tab.data-api', '[data-toggle="tab"], [data-tab]', function(e) {
        e.preventDefault()
        $(this).tab('show')
    })
...
+ function($) {
    ...
    Collapse.prototype.show = function() {
        ...
        var startEvent = $.Event('show.' + mguiname)
        ...
        if(actives && actives.length) {
            var hasData = actives.data(mguiname)
            if(hasData && hasData.transitioning) return
            actives.collapse('hide')
            hasData || actives.data(mguiname, null)
        }
        ...
        var complete = function() {
            ...
            this.$element.trigger('shown.' + mguiname)
        }
    ...
    Collapse.prototype.hide = function() {
        ...
        var startEvent = $.Event('hide.' + mguiname)
        this.$element.trigger(startEvent)
        if(startEvent.isDefaultPrevented()) return
    ...
    $.fn.collapse = function(option) {
        return this.each(function() {
            ...
            if(!data) $this.data(mguiname, (data = new Collapse(this, options)))
            if(typeof option == 'string') data[option]()
        })
    }
    ...
    $(document).on('click.' + mguiname + '.data-api', '[data-toggle=collapse]', function(e) {
        var $this = $(this),
            href
        var target = $this.attr('data-target') || e.preventDefault() || (href = $this.attr('href')) && href.replace(/.*(?=#[^\s]+$)/, '') //strip for ie7
        var $target = $(target)
        var data = $target.data(mguiname)
...
(function($) {
    ...
    Browser.prototype.tip = function(showCoontent) {
        ...
        $browseHappy.find('.content').html(showCoontent || this.browseHappyTip || browseHappyTip[$.mgui.clientLang() || 'zh_cn']);
    };
    ...
    $(function() {
        if(!$('body').hasClass('disabled-browser-tip')) {
            if($.mgui.browser.ie && $.mgui.browser.ie < 8) {
                $.mgui.browser.tip();
            }
        }
    });
...
+ function($) {
    ...
    function ScrollSpy(element, options) {
        ...
        this.$body = $('body')
        this.$scrollElement = this.$element.on('scroll.' + mguiname + '.data-api', process)
    ...
    ScrollSpy.prototype.activate = function(target) {
        ...
        active.trigger('activate.' + mguiname)
    }
    ...
    $.fn.scrollspy = function(option) {
        return this.each(function() {
            ...
            if(!data) $this.data(mguiname, (data = new ScrollSpy(this, options)))
            if(typeof option == 'string') data[option]()
        })
    }
...
(function($) {
    ...
    var NAME = 'mgui.searchBox'; // modal name
    ...
    var SearchBox = function(element, options) {
        ...
        if ($input.length) {
            ...
            console.error('MGUI: search box init error, cannot find search box input element.');
        }
    };
...
(function($, document) {
    ...
    Draggable.prototype.init = function() {
        ...
        var mouseDown = function(event) {
            var mouseButton = $.mgui.getMouseButtonCode(setting.mouseButton);
...
(function($, document, Math) {
    ...
    Droppable.prototype.trigger = function(name, params) {
        return $.mgui.callEvent(this.options[name], params, this);
    };
 ...
 * ========================================================================
 * Updates in MGUI：
 * 1. changed event namespace to *.mgui.modal
 * 2. added position option to ajust poisition of modal
 * 3. added event 'escaping.mgui.modal' with an param 'esc' to judge the esc
 *    key down
...
+ function($, undefined) {
        ...
        if(options.remote) {
            that.$element
                ...
                .load(options.remote, function() {
                    that.$element.trigger('loaded.' + mguiname)
                })
        }
    }
        ...
        if($dialog.hasClass('modal-moveable')) {
            ...
            if(rememberPos) {
                if(rememberPos === true) {
                    ...
                    pos = $.mgui.store.pageGet(mguiname + '.rememberPos.' + rememberPos);
                }
            }
            ...
            $dialog.draggable({
                ...
                finish: function(e) {
                    ...
                    if(rememberPos) {
                        ...
                        if($.mgui.store && rememberPos !== true) {
                            $.mgui.store.pageSet(mguiname + '.rememberPos.' + rememberPos, e.pos);
                        }
                    }
                },
    ...
    Modal.prototype.show = function(_relatedTarget, position) {
        ...
        that.$element.on('click.dismiss.' + mguiname, '[data-dismiss="modal"]', $.proxy(that.hide, that))
    ...
    Modal.prototype.hide = function(e) {
        ...
        $(document).off('focusin.' + mguiname)

        that.$element
            .removeClass('in')
            .attr('aria-hidden', true)
            .off('click.dismiss.' + mguiname)
    ...
    Modal.prototype.enforceFocus = function() {
        $(document)
            .off('focusin.' + mguiname) // guard against infinite focus loop
            .on('focusin.' + mguiname, $.proxy(function(e) {
    ...
    Modal.prototype.escape = function() {
        if(this.isShown && this.options.keyboard) {
            $(document).on('keydown.dismiss.' + mguiname, $.proxy(function(e) {
                if(e.which == 27) {
                    var et = $.Event('escaping.' + mguiname)
            ...
            $(document).off('keydown.dismiss.' + mguiname)
        }
    }
    ...
    Modal.prototype.hideModal = function() {
        ...
        this.backdrop(function() {
            that.$element.trigger('hidden.' + mguiname)
        })
    }
    ...
    Modal.prototype.backdrop = function(callback) {
        ...
        if(this.isShown && this.options.backdrop) {
            ...
            this.$element.on('mousedown.dismiss.' + mguiname, $.proxy(function(e) {
    ...
    function Plugin(option, _relatedTarget, position) {
        return this.each(function() {
            ...
            if(!data) $this.data(mguiname, (data = new Modal(this, options)))
    ...
    $(document).on('click.' + mguiname + '.data-api', '[data-toggle="modal"]', function(e) {
        var $this = $(this)
        ...
        var option = $target.data(mguiname) ? 'toggle' : $.extend({
            remote: !/#/.test(href) && href
        }, $target.data(), $this.data())

        if($this.is('a')) e.preventDefault()

        $target.one('show.' + mguiname, function(showEvent) {
            // only register focus restorer if modal will actually get shown
            if(showEvent.isDefaultPrevented()) return
            $target.one('hidden.' + mguiname, function() {
...
(function($, window, undefined) {
    ...
    var NAME = 'mgui.modaltrigger',
        STR_AJAX = 'ajax',
        MGUI_MODAL = '.mgui.modal',
        STR_STRING = 'string';
    ...
    ModalTrigger.prototype.init = function(options) {
        ...
        if($modal.length) {
            if(!that.isShown) $modal.off(MGUI_MODAL);
            $modal.remove();
        }
        ...
        var bindEvent = function(optonName, eventName) {
            ...
            if($.isFunction(handleFunc)) $modal.on(eventName + MGUI_MODAL, handleFunc);
        };
        ...
        $modal.on('shown' + MGUI_MODAL, function() {
            that.isShown = true;
        }).on('hidden' + MGUI_MODAL, function() {
            that.isShown = false;
        });
                    ...
                    success: function(data) {
                        try {
                            ...
                            if (window.console && window.console.warn) {
                                console.warn('MGUI: Cannot recogernize remote content.', {error: e, data: data});
                            }
                            $modal.html(data);
                        }
                        $modal.callComEvent(that, 'loaded', {
    ...
    ModalTrigger.prototype.close = function(callback, redirect) {
        if(callback || redirect) {
            this.$modal.on('hidden' + MGUI_MODAL, function() {
 ...
 * http://twmgui.github.com/bootstrap/javascript.html#tooltip
...
+ function($) {
    ...
    Tooltip.prototype.enter = function(obj) {
        var self = obj instanceof this.constructor ?
            obj : $(obj.currentTarget)[this.type](this.getDelegateOptions()).data('mgui.' + this.type)
    ...
    Tooltip.prototype.show = function(content) {
        var e = $.Event('show.mgui.' + this.type)
        ...
        if((content || this.hasContent()) && this.enabled) {
            ...
            var complete = function () {
                ...
                that.$element.trigger('shown.mgui.' + that.type)
    ...
    Tooltip.prototype.hide = function() {
        var that = this
        var $tip = this.tip()
        var e = $.Event('hide.mgui.' + this.type)
        ...
        this.$element.trigger('hidden.mgui.' + this.type)
    ...
    Tooltip.prototype.toggle = function(e) {
        var self = e ? $(e.currentTarget)[this.type](this.getDelegateOptions()).data('mgui.' + this.type) : this
        self.tip().hasClass('in') ? self.leave(self) : self.enter(self)
    }

    Tooltip.prototype.destroy = function() {
        this.hide().$element.off('.' + this.type).removeData('mgui.' + this.type)
    }
    ...
    $.fn.tooltip = function(option, params) {
        return this.each(function() {
            var $this = $(this)
            var data = $this.data('mgui.tooltip')
            var options = typeof option == 'object' && option

            if(!data) $this.data('mgui.tooltip', (data = new Tooltip(this, options)))
...
+ function($) {
    ...
    $.fn.popover = function(option) {
        return this.each(function() {
            var $this = $(this)
            var data = $this.data('mgui.popover')
            var options = typeof option == 'object' && option

            if(!data) $this.data('mgui.popover', (data = new Popover(this, options)))
            if(typeof option == 'string') data[option]()
        })
    }
...
+ function($) {
    ...
    var mguiname = 'mgui.dropdown';
    var backdrop = '.dropdown-backdrop'
    var toggle = '[data-toggle=dropdown]'
    var Dropdown = function(element) {
        var $el = $(element).on('click.' + mguiname, this.toggle)
    }
    ...
    Dropdown.prototype.toggle = function(e) {
        ...
        if(!isActive) {
            ...
            $parent.trigger(e = $.Event('show.' + mguiname))

            if(e.isDefaultPrevented()) return

            $parent
                .toggleClass('open')
                .trigger('shown.' + mguiname)
    ...
    function clearMenus() {
        ...
        $(toggle).each(function(e) {
            ...
            $parent.trigger(e = $.Event('hide.' + mguiname))
            if(e.isDefaultPrevented()) return
            $parent.removeClass('open').trigger('hidden.' + mguiname)
        })
    }
...
(function($, undefined) {
    ...
    var NAME = 'mgui.contextmenu'; // model name
    ...
    var ContextMenu = {};
    var targetId = 'mgui-contextmenu-' + $.mgui.uuid();
 ...
 * http://twmgui.github.com/bootstrap/javascript.html#carousel
 *
 * MGUI: The file has been changed in MGUI. It will not keep update with the
 * Bootsrap version in the future.
 * 
...
+ function($) {
    ...
    Carousel.prototype.slide = function(type, next) {
        ...
        var e = $.Event('slide.mgui.carousel', {
    ...
    $.fn.carousel = function(option) {
        return this.each(function() {
            var $this = $(this)
            var data = $this.data('mgui.carousel')
            var options = $.extend({}, Carousel.DEFAULTS, $this.data(), typeof option == 'object' && option)
            var action = typeof option == 'string' ? option : options.slide

            if(!data) $this.data('mgui.carousel', (data = new Carousel(this, options)))
    ...
    $(document).on('click.mgui.carousel.data-api', '[data-slide], [data-slide-to]', function(e) {
        var $this = $(this),
            href
        ...
        if(slideIndex = $this.attr('data-slide-to')) {
            $target.data('mgui.carousel').to(slideIndex)
        }
...
(function($, window, Math) {
    ...
    if(!$.mgui.imgReady) throw new Error('imgReady requires for lightbox');
    ...
    Lightbox.prototype.bindEvents = function() {
        ...
        $e.modalTrigger({
            ...
            custom: function(e) {
                ...
                $.mgui.imgReady(options.image, function() {
                    dialog.css({
    ...
    $.fn.lightbox = function(option) {
        ...
        return this.each(function() {
            ...
            var data = $this.data('mgui.lightbox');
            if(!data) $this.data('mgui.lightbox', (data = new Lightbox(this, options)));
...
(function($, window, undefined) {
    ...
    var hideMessage = function() {
        $('.messager').each(function() {
            var msg = $(this).data('mgui.messager');
    ...
    $.each({
        ...
        mguiMessager[name] = function(message, options) {
...
(function($) {
    ...
    Tree.prototype._init = function() {
        ...
        var isPreserveEnable = $.mgui && $.mgui.store && $.mgui.store.enable;
        if(isPreserveEnable) {
            this.selector = name + '::' + (options.name || '') + '#' + (this.$.attr('id') || globalId++);
            this.store = $.mgui.store[options.name ? 'get' : 'pageGet'](this.selector, {});
        }
    ...
    Tree.prototype.preserve = function($li, id, expand) {
        ...
        if($li) {
            ...
            $.mgui.store[this.options.name ? 'set' : 'pageSet'](this.selector, this.store);
        } else {
            var that = this;
            this.store = {};
